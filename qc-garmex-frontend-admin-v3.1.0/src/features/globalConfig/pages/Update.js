import React from 'react'
import CIcon from '@coreui/icons-react'
import {
    CCard, CCardHeader, CCardBody, CCardFooter, CCol, CRow, CLabel, CForm, CFormGroup, CInput, CFormText, CFade, CButton
} from '@coreui/react'

const UpdateGlobalConfig = () => {
    const [showElements] = React.useState(true);

    return (
        <>
            <CRow>
                <CCol xs="12">
                    <CFade timeout={300} in={showElements} unmountOnExit={true}>
                        <CCard>
                            <CCardHeader className="bg-secondary" style={{ color: '#000' }}>
                                <h6>Chỉnh sửa cấu hình</h6>
                            </CCardHeader>
                            <CCardBody>
                                <CForm action="" method="post">
                                    <CRow>
                                        <CCol xs="12" md="6">
                                            <CFormGroup>
                                                <CLabel htmlFor="nf-id">Id</CLabel>
                                                <CInput
                                                    type="text"
                                                    id="nf-id"
                                                    name="nf-id"
                                                    placeholder=""
                                                    autoComplete="id"
                                                    value="ff8d9763-0173-4c8e-c8c6def11f7e"
                                                    disabled={true}
                                                />
                                            </CFormGroup>
                                        </CCol>
                                        <CCol xs="12" md="6">
                                            <CFormGroup>
                                                <CLabel htmlFor="nf-key">Key <span style={{ color: 'red' }}>*</span></CLabel>
                                                <CInput
                                                    type="text"
                                                    id="nf-key"
                                                    name="nf-key"
                                                    autoComplete="key"
                                                />
                                                <CFormText className="help-block">Vui lòng nhập key ...</CFormText>
                                            </CFormGroup>
                                        </CCol>
                                    </CRow>
                                    <CRow>
                                        <CCol xs="12" md="6">
                                            <CFormGroup>
                                                <CLabel htmlFor="nf-value">Giá trị <span style={{ color: 'red' }}>*</span></CLabel>
                                                <CInput
                                                    type="text"
                                                    id="nf-value"
                                                    name="nf-value"
                                                    autoComplete="value"
                                                />
                                                <CFormText className="help-block">Vui lòng nhập Giá trị ...</CFormText>
                                            </CFormGroup>
                                        </CCol>
                                        <CCol xs="12" md="6">
                                            <CFormGroup>
                                                <CLabel htmlFor="nf-configTypeId">Loại cấu hình <span style={{ color: 'red' }}>*</span></CLabel>
                                                <CInput
                                                    type="text"
                                                    id="nf-configTypeId"
                                                    name="nf-configTypeId"
                                                    autoComplete="configTypeId"
                                                />
                                                <CFormText className="help-block">Vui lòng nhập loại cấu hình ...</CFormText>
                                            </CFormGroup>
                                        </CCol>
                                    </CRow>
                                    <CRow>
                                        <CCol xs="12" md="6">
                                            <CFormGroup>
                                                <CLabel htmlFor="nf-description">Diễn giải <span style={{ color: 'red' }}>*</span></CLabel>
                                                <CInput
                                                    type="description"
                                                    id="nf-description"
                                                    name="nf-description"
                                                    autoComplete="description"
                                                />
                                                <CFormText className="help-block">Vui lòng nhập diễn giải ...</CFormText>
                                            </CFormGroup>
                                        </CCol>
                                        <CCol xs="12" md="6">
                                            <CFormGroup>
                                                <CLabel htmlFor="nf-status">Trạng thái <span style={{ color: 'red' }}>*</span></CLabel>
                                                <CInput
                                                    type="status"
                                                    id="nf-status"
                                                    name="nf-status"
                                                    autoComplete="status"
                                                    value="Chưa kích hoạt"
                                                />
                                                <CFormText className="help-block">Vui lòng chọn hoặc nhập trạng thái ...</CFormText>
                                            </CFormGroup>
                                        </CCol>
                                    </CRow>
                                                               
                                    <CRow>
                                        <CCol xs="12" md="6">
                                            <CFormGroup>
                                                <CLabel htmlFor="nf-creator">Người tạo</CLabel>
                                                <CInput
                                                    type="text"
                                                    id="nf-creator"
                                                    name="nf-creator"
                                                    disabled
                                                />
                                            </CFormGroup>
                                        </CCol>
                                        <CCol xs="12" md="6">
                                            <CFormGroup>
                                                <CLabel htmlFor="nf-created-day">Ngày tạo</CLabel>
                                                <CInput
                                                    type="date"
                                                    id="nf-created-day"
                                                    name="nf-created-day"
                                                    placeholder="date"
                                                />
                                            </CFormGroup>
                                        </CCol>
                                    </CRow>
                                    <CRow>
                                        <CCol xs="12" md="6">
                                            <CFormGroup>
                                                <CLabel htmlFor="nf-editor">Người chỉnh sửa</CLabel>
                                                <CInput
                                                    type="text"
                                                    id="nf-editor"
                                                    name="nf-editor"
                                                    disabled
                                                />
                                            </CFormGroup>
                                        </CCol>
                                        <CCol xs="12" md="6">
                                            <CFormGroup>
                                                <CLabel htmlFor="nf-edited-day">Ngày chỉnh sửa</CLabel>
                                                <CInput
                                                    type="date"
                                                    id="nf-edited-day"
                                                    name="nf-edited-day"
                                                    placeholder="date"
                                                />
                                            </CFormGroup>
                                        </CCol>
                                    </CRow>
                                </CForm>
                            </CCardBody>
                            <CCardFooter>
                                <CButton type="submit" size="sm" color="primary"><CIcon name="cil-scrubber" /> Gửi đi</CButton>&nbsp;
 <CButton type="cancel" size="sm" color="danger"><CIcon name="cil-ban" /> Bỏ qua</CButton>
                            </CCardFooter>
                        </CCard>
                    </CFade>
                </CCol>
            </CRow>
        </>
    )
}

export default UpdateGlobalConfig;
